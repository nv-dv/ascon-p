...
inline dshare_128 dS_AND(dshare_128* p1, dshare_128* p2) {
	#ifdef REFRESH
...
inline void dS_ROUND(int C) {
	...
	//t0 = ~x0;    t1 = ~x1;    t2 = ~x2;    t3 = ~x3;    t4 = ~x4;
	for (int j = 0; j < MASKING_ORDER/2; j++)
	{
		asm("vmovdqa xmm0, %0"::"m"(x[0].s[j]));
		asm("vmovdqa xmm1, %0"::"m"(x[1].s[j]));
		asm("vmovdqa xmm2, %0"::"m"(x[2].s[j]));
		asm("vmovdqa xmm3, %0"::"m"(x[3].s[j]));
		asm("vmovdqa xmm4, %0"::"m"(x[4].s[j]));
		asm("vmovdqa %0, xmm0":"=m"(t[0].s[j]));
		asm("vmovdqa %0, xmm1":"=m"(t[1].s[j]));
		asm("vmovdqa %0, xmm2":"=m"(t[2].s[j]));
		asm("vmovdqa %0, xmm3":"=m"(t[3].s[j]));
		asm("vmovdqa %0, xmm4":"=m"(t[4].s[j]));
	}	
	t[0].u64ptr[0] ^= C_not;
	t[1].u64ptr[0] ^= C_not;
	t[2].u64ptr[0] ^= C_not;
	...
	for (int i = 0; i < 5; i++)
	{
		...
		for (size_t j = 0; j<MASKING_ORDER/2; j++) {
			asm("vmovdqa xmm0, %0"::"m"(x[i].s[j]));
			asm("vxorpd xmm0, xmm0, %0"::"m"(t[tmp].s[j]));
			asm("vmovdqa %0, xmm0":"=m"(x[i].s[j]));
		}
	}
	//x1 ^= x0;    x0 ^= x4;    x3 ^= x2;    x2 = ~x2;
	for (size_t i=0; i<MASKING_ORDER/2; i++) {
		asm("vmovdqa xmm0, %0"::"m"(x[1].s[i]));
		asm("vmovdqa xmm1, %0"::"m"(x[0].s[i]));
		asm("vmovdqa xmm2, %0"::"m"(x[3].s[i]));
		asm("vxorpd xmm0, xmm0, %0"::"m"(x[0].s[i]));
		asm("vxorpd xmm1, xmm1, %0"::"m"(x[4].s[i]));
		asm("vxorpd xmm2, xmm2, %0"::"m"(x[2].s[i]));
		asm("vmovdqa %0, xmm0":"=m"(x[1].s[i]));
		asm("vmovdqa %0, xmm1":"=m"(x[0].s[i]));
		asm("vmovdqa %0, xmm2":"=m"(x[2].s[i]));
	}
	x[2].u64ptr[0] ^= C_not;
	// linear layer

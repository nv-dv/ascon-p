...
/* main function */
void ascon12 (/*inputs*/ DATATYPE input__[5][MASKING_ORDER], /*outputs*/ DATATYPE output__[5][MASKING_ORDER]) {
...
void bench_speed(size_t count) {
  if (count == 0) {
    // init shares...
    for (size_t i = 0; i < 5; ++i)
      input__[i][0] = i;
  }
  ascon12(input__,input__);
}

void usuba_get_shares() {
  DATATYPE res[5] = {0};
  for (int i = 0; i < MASKING_ORDER; ++i)
  {
    res[0] ^= input__[0][i]; res[1] ^= input__[1][i]; res[2] ^= input__[2][i]; res[3] ^= input__[3][i]; res[4] ^= input__[4][i];
  }

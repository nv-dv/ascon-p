...
inline void _8S_AND(share8* share0, share8* share1, share8* res_share, int i, int j) {
    ...
    __m512i rtag2 = _mm512_loadu_si512(&RandIn[i+2]);
    // input refresh
    __m512i refr = _mm512_loadu_si512(&RandIn[i+3]);
    ...
    //x.rot(y,4)
    z2 = _mm512_and_epi64(x, y);
    z2 =  _mm512_xor_epi64(z2, rtag2);
    rtag2 = _zmm_rotr(rtag2);
    res = _mm512_xor_epi64(res, z2);
    y = _zmm_rotr(y);
    res = _mm512_xor_epi64(res, rtag2);
    // write out
...
void _8S_ROUND(uint64_t c) {
    ...
    _8S_AND(&ssst1, &sssx2, &ssst1, 4,3);
    _8S_AND(&ssst2, &sssx3, &ssst2, 8,6);
    _8S_AND(&ssst3, &sssx4, &ssst3, 12,9);
    _8S_AND(&ssst4, &sssx0, &ssst4, 16,12);
    /*load state*/
    DD[7] = C_not;
